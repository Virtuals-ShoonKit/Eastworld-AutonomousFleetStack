...
void LIVMapper::ReadParameters()
{
  ...
  node_->declare_parameter("uav.publish_map_odom_tf", true);
  ...
  publish_map_odom_tf_ = node_->get_parameter("uav.publish_map_odom_tf").as_bool();
...
void LIVMapper::InitializeSubscribersAndPublishers()
{
  ...
  pub_imu_prop_odom_ =
      node_->create_publisher<nav_msgs::msg::Odometry>("/imu_propagate", qos);
  pub_clock_ =
...
void LIVMapper::SavePCD()
{
      ...
      pcd_save_interval_ < 0)
  {
    ...
    else
    {
      ...
      pcl::VoxelGrid<PointXYZIN> voxel_filter;
      voxel_filter.setInputCloud(pcl_wait_save_intensity_);
      voxel_filter.setLeafSize(filter_size_pcd_, filter_size_pcd_, filter_size_pcd_);
      voxel_filter.filter(*downsampled_cloud);

      pcd_writer.writeBinary(raw_points_dir, *downsampled_cloud);
      std::cout << GREEN << "Point cloud data saved to: " << raw_points_dir
                << " with point count: " << downsampled_cloud->points.size()
                << " (voxel filtered from " << pcl_wait_save_intensity_->points.size()
                << " at " << filter_size_pcd_ << "m)" << RESET << std::endl;
    }
  }
}
...
void LIVMapper::ImuPropCallback()
{
  ...
  if (imu_prop_enable_ && !prop_imu_buffer_.empty())
  {
    ...
    // IMU-propagated odom for EKF fusion (body-frame orientation)
    ...
    // Broadcast odom->base_link TF for Nav2 (50 Hz).
    // map->odom identity is now a static TF published in the launch file
    // so it never expires when this callback is blocked by LIO processing.
    ...
    odom_tf.transform.rotation.w = q_body.w();
    odom_tf.transform.rotation.x = q_body.x();
    odom_tf.transform.rotation.y = q_body.y();
    odom_tf.transform.rotation.z = q_body.z();

    std::vector<geometry_msgs::msg::TransformStamped> transforms;
    transforms.push_back(odom_tf);

    if (publish_map_odom_tf_) {
      geometry_msgs::msg::TransformStamped map_odom_tf;
      map_odom_tf.header.stamp = imu_prop_odom_.header.stamp;
      map_odom_tf.header.frame_id = "map";
      map_odom_tf.child_frame_id = "odom";
      map_odom_tf.transform.rotation.w = 1.0;
      transforms.push_back(map_odom_tf);
    }

    tf_broadcaster_->sendTransform(transforms);

    rosgraph_msgs::msg::Clock clock_msg;
    clock_msg.clock = imu_prop_odom_.header.stamp;
...
void LIVMapper::ImuCbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  {
    static double latched_offset = 0.0;
    static bool   offset_latched = false;
    if (!offset_latched && last_timestamp_lidar_ > 0.0)
    {
      latched_offset = std::round(last_timestamp_lidar_ - timestamp);
      offset_latched = true;
      last_timestamp_imu_ = 0.0;
      imu_buffer_.clear();
      RCLCPP_INFO(node_->get_logger(),
                  "Latched IMU-LiDAR epoch offset: %.0f s (raw delta %.3f)",
                  latched_offset, last_timestamp_lidar_ - timestamp);
    }
    if (!offset_latched)
      return;
    timestamp += latched_offset;
  }
...
void LIVMapper::PublishFrameWorld(VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.stamp = imu_prop_odom_.header.stamp;
...
void LIVMapper::PublishOdometry()
{
  ...
  odom_aft_mapped_.child_frame_id = "imu_link";
  ...
  // TF for odom->base_link is broadcast at 50 Hz in ImuPropCallback();
  // no additional TF broadcast needed here.
}
