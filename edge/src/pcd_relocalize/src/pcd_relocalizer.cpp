...
void PcdRelocalizer::cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
  ...
  *accumulated_cloud_ += incoming;
  ...
  double elapsed = (now() - accumulation_start_).seconds();
...
void PcdRelocalizer::runRelocalization() {
  pcl::VoxelGrid<pcl::PointXYZ> vg;
...
Eigen::Isometry3d PcdRelocalizer::icpRefine(
  ...
  auto result = small_gicp::align(tgt_pts, src_pts, initial_guess, setting);

  if (!result.converged) {
    RCLCPP_WARN(get_logger(), "GICP did not converge — using TEASER++ result");
    return initial_guess;
  }

  RCLCPP_INFO(get_logger(), "GICP converged in %zu iterations, %zu inliers",
              result.iterations, result.num_inliers);
...
void PcdRelocalizer::publishMapToOdom(const Eigen::Isometry3d& T) {
  ...
  RCLCPP_INFO(get_logger(),
              "Published map->odom TF: t=(%.3f, %.3f, %.3f) q=(%.4f, %.4f, %.4f, %.4f)",
