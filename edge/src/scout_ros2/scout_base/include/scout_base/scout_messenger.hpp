...
class ScoutMessenger {
 public:
  ...
  void SetPublishOdomTF(bool publish) { publish_odom_tf_ = publish; }
  ...
  void PublishStateToROS() {
    ...
    for (int i = 0; i < 4; ++i) {
      // Keep ROS array indexing stable by motor slot.
      // Some firmware frames may arrive out-of-phase, so using the embedded
      // motor_id as an index can temporarily collapse all values into slot 0.
      status_msg.actuator_states[i].motor_id = static_cast<uint8_t>(i);
 ...
 private:
  ...
  void PublishOdometryToROS(const MotionStateMessage &msg, double dt) {
    ...
    if (publish_odom_tf_) tf_broadcaster_->sendTransform(tf_msg);
